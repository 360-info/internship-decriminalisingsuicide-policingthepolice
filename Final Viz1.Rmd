...
for sf- http://www.diva-gis.org/datadown
...
```{r}
india_regional_2019 <-
read_csv("data/map/RS_Session_253_A_211.1.csv") %>%
...
states_merged <- inner_join(states_shape_sf, india_regional_2019,
  by = c("ID_1" = "id"))
```
...
map_india <- ggplot() +
  ...
  labs(
    ...
    subtitle = "Registered suicides in 2019",
      caption = paste(
      "**CHART:** Nishtha Arora & James Goldie, 360info",
      "**DATA:** data.gov.in",
      sep = "<br>")
  ...
  scale_fill_continuous(name = "No. of suicides", low = "lightblue", high = "#A076F8") 
...
```{r}
pakistan <- suicide_rates %>% filter(Entity == "Pakistan") 
```

```{r}
plot_pak <- ggplot(pakistan, aes(x = Year, y = `Deaths - Self-harm - Sex: Both - Age: Age-standardized (Rate)`)) +
  geom_line(color="brown") +
  geom_point(color = "brown", size = 1.5) +
  # geom_vline(xintercept = 2007, linetype = "dashed") +
  # geom(aes(x = 2005, y = 18.5, label = "Suicide Rate", colour = "Black")) +
  ...
  labs(
    ...
    subtitle="Suicide rates in Pakistan have fallen/grown by ________",
    caption = paste(
      "**CHART:** Nishtha Arora & James Goldie, 360info",
      "**DATA:** Our World in Data",
      sep = "<br>"
...
save_360plot(plot_pak, "paktimeseries.pdf")
knitr::include_graphics("paktimeseries.png")
```
