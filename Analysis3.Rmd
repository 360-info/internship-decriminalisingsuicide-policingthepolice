...
write.csv(india_map, "data/statemap.csv")
...
```{r}
state_map_edited <- read_csv("data/statemap_editedmanually.csv")
```
...
```{r}
Merged_data = merge(fortify_shape, state_map_edited, by="id", all.x=TRUE)
...
ggplot() +
  ...
  coord_map()+
  scale_fill_gradient(name="State", limits=c(0,100), low = 'white', high = 'red')+
  labs(title="Average Suicide Rate: India 2000-2012")+
  xlab('Longitude')+
  ylab('Latitude')
