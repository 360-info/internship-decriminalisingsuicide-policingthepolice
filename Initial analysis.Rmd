...
```{r}
global_suicide_rates <-
  read_csv("data/Article-wise/data.csv") %>%
...
global_suicide_rates %>%
  filter(Location %in% c(
    "Kiribati", "Central African Republic", "Latvia", "Republic of Korea",
    "Fiji", "Rwanda", "Poland", "Comoros", "Uzbekistan",
    "Bosnia and Herzegovina", "Grenada", "Niger", "Cuba", "Equatorial Guinea",
    "Burkina Faso", "Samoa", "Latvia", "Sao Tome and Principe", "Honduras",
    "Lebanon", "Maldives", "Bahamas", "Timor-Leste", "Iraq",
    "Dominican Republic", "Iran (Islamic Republic of)", "Brazil",
    "Bolivia (Plurinational State of)",
    "The former Yugoslav Republic of Macedonia", "Portugal", "Belize",
    "Serbia", "Mali", "Argentina", "United Republic of Tanzania",
...
```{r}
bangladesh <- global_suicide_rates %>% filter(Location == "Bangladesh")
...
Suicides started increasing from 2016
...
```{r}
selected_suicide_rates <-
  global_suicide_rates %>% 
  filter(Location %in% c("India", "Maldives", "Afghanistan", "Nepal",
...
```{r}
criminalised_countries <- global_suicide_rates %>%
  filter(Location %in% c(
    "Bahamas", "Bangladesh", "Guyana", "Ghana", "Kenya", "Malawi", "Malaysia",
    "Myanmar", "Nigeria", "Pakistan", "Papua New Guinea", "Qatar",
    "Saint Lucia", "Somalia", "South Sudan", "Sudan", "Tanzania", "Tonga",
    "Uganda", "Brunei"))

ggplot(
  criminalised_countries,
  aes(colour = Location, y = FactValueNumeric, x = Period)) +
  geom_line()
```
